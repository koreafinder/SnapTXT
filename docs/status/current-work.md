...
_최종 업데이트: 2026-03-04 07:37 (Event Replay 분포 검증 90% 달성)_

## 🎯 핵심 브레이크스루: Phase 3.0 Event Replay 분포 검증 성공! (2026-03-04)

### ✅ **분포 검증 Coverage 0% → 90% 달성**
- **🔧 Coverage**: 0% → **90%** (9/10 패턴 매칭) ✅
- **📊 KL divergence**: 16.22 → **1.53** (크게 개선) ✅  
- **📈 Jensen-Shannon**: NaN → **0.22** (계산 가능) ✅
- **📉 Spearman correlation**: NaN → **0.61** (계산 가능하지만 목표 0.85 미달)
- **🧪 합성 샘플 유효율**: **92.0%** (46/50)
- **⚙️ Signature 적용 성공률**: **79.8%** (75/94)

### 🔨 해결된 기술 문제들
1. **`_validate_distribution` 로직**: `applied_events`에서 직접 signature 추출
2. **JSON serialization 오류**: boolean → string 변환
3. **분포 검증 파일 생성**: real/synth 비교표 자동 생성
...
_최종 업데이트: 2026-03-04 22:20 (문서 정리 및 방향성 재정립)_
...
### **✅ 실험 완료됐지만 실제 통합 안된 것들**
- **Google Vision API**: 4,270자 자동 추출 (45분→2분) → **별도 UI로만 존재**
- **Book Fingerprint**: Book ID 7717f6c549c5 생성 → **standalone 테스트만**
- **Book Profile YAML**: GPT 교정 기준 생성 → **run_pipeline()과 미연결**

## 🚨 남은 개선 과제

### **Spearman correlation 향상 (현재 0.61 → 목표 0.85)**
- INSERT 계열 signature 적용률 개선 필요:
  - `INSERT[","]`: 선택=8, 적용=0 (0%)  
  - `INSERT["."]`: 선택=9, 적용=3 (33%)
- GT 텍스트 다양성 확대 (현재 2개 문단)
- Error event 역변환 알고리즘 정밀도 향상

### **📁 생성된 분석 파일**
```
.snaptxt/analysis/
├── error_events_20260304_073728.jsonl       # 86개 실제 오류 이벤트
├── synthetic_replay_dataset_20260304_073728.jsonl  # 46개 합성 샘플
├── distribution_validation_20260304_073728.json    # 분포 검증 결과
├── real_top10_signatures_20260304_073728.txt      # 실제 패턴 분포
├── synth_top10_signatures_20260304_073728.txt     # 합성 패턴 분포  
└── distribution_comparison_20260304_073728.txt    # 비교 분석 테이블
```
...
### **1순위: Spearman correlation 개선 (0.61 → 0.85)** (1시간)
**문제**: INSERT 계열 signature 적용률 낮음
- `INSERT[","]`: 선택=8, 적용=0 (0%)
- `INSERT["."]`: 선택=9, 적용=3 (33%)

**해결 방안**:
1. INSERT 계열 signature 적용 로직 개선
2. GT 텍스트 다양성 확대 (현재 2개 문단 → 5+개)
3. Error event 역변환 알고리즘 정밀도 향상

### **2순위: Top200 패턴 확장 테스트** (30분)
```bash
python build_error_replay_dataset.py --folder samples --topk 200 --synthetic-size 1000
```

### **3순위: Production 검증 완료** (20분)
    extracted_text,
    book_profile=get_or_create_book_profile(extracted_text),  # ← 자동 연결
    stage2_config=Stage2Config(),
    stage3_config=Stage3Config()
)
```

### **3순위: 실제 CER 개선 측정** (10분)
```
실제 책 1권으로 Before/After 테스트 실행
→ Phase 2 통합 효과가 실제로 나오는지 확인
→ +2.22%p를 넘어서는 개선량 달성 목표
```
