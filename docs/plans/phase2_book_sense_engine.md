...
### **2️⃣ GPT Integration** (1회성 개입)

**목적**: 책 교정 기준만 생성, 교정 엔진 아님

```python
class GPTBookAnalyzer:
    def generate_book_profile(self, ocr_samples: List[str], fingerprint: BookFingerprint) -> dict:
        """GPT에게 책 샘플 5페이지만 보여주고 교정 기준 생성"""
        
        prompt = f"""
        다음 OCR 결과들을 분석하여 이 책의 교정 기준을 생성해주세요:
        
        OCR 샘플들:
        {ocr_samples[:5]}  # 최대 5페이지만
...
### **Day 1-2: Book Fingerprint System**
- [ ] BookFingerprinter 클래스 구현
- [ ] Typography/Spacing/Punctuation 패턴 추출기
- [ ] Book Hash 생성 알고리즘
- [ ] 테스트: 동일 책 감지 정확도

### **Day 3: GPT Integration**  
- [ ] GPTBookAnalyzer 클래스 구현
- [ ] GPT 프롬프트 최적화
- [ ] API 호출 관리 (1회 제한)
- [ ] 테스트: GPT 응답 품질 검증
